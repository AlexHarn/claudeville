...
def print_header():
    ...
    print()
    print(c("╔═══════════════════════════════════════════════════════════════╗", Colors.CYAN))
    print(c("║", Colors.CYAN) + c("              CLAUDEVILLE SIMULATION ENGINE                   ", Colors.BRIGHT_WHITE, Colors.BOLD) + c("║", Colors.CYAN))
    print(c("║", Colors.CYAN) + c("         Generative Agents powered by Claude CLI              ", Colors.DIM) + c("║", Colors.CYAN))
    print(c("╚═══════════════════════════════════════════════════════════════╝", Colors.CYAN))
    print()
...
def print_sim_info(sim_code, fork_code, curr_time, step, personas):
    ...
    print(c("┌─ Simulation Info ─────────────────────────────────────────────┐", Colors.BRIGHT_BLACK))
    ...
    print(c("│", Colors.BRIGHT_BLACK) + f" Time: {c(curr_time.strftime('%B %d, %Y %H:%M'), Colors.YELLOW)}")
    print(c("│", Colors.BRIGHT_BLACK) + f" Step: {c(str(step), Colors.GREEN)}")
    print(c("│", Colors.BRIGHT_BLACK) + f" Personas: {c(', '.join(personas), Colors.MAGENTA)}")
...
def print_help():
    ...
    print(c("┌─ Commands ────────────────────────────────────────────────────┐", Colors.BRIGHT_BLACK))
    print(c("│", Colors.BRIGHT_BLACK))
    print(c("│", Colors.BRIGHT_BLACK) + c("  Simulation Control:", Colors.BRIGHT_WHITE, Colors.BOLD))
    print(c("│", Colors.BRIGHT_BLACK) + f"    {c('run <N>', Colors.GREEN)}           Run N simulation steps")
    print(c("│", Colors.BRIGHT_BLACK) + f"    {c('save', Colors.GREEN)}              Save current progress")
    print(c("│", Colors.BRIGHT_BLACK) + f"    {c('fin', Colors.GREEN)}               Save and exit")
    print(c("│", Colors.BRIGHT_BLACK) + f"    {c('exit', Colors.RED)}              Exit without saving")
    print(c("│", Colors.BRIGHT_BLACK))
    print(c("│", Colors.BRIGHT_BLACK) + c("  Status:", Colors.BRIGHT_WHITE, Colors.BOLD))
    print(c("│", Colors.BRIGHT_BLACK) + f"    {c('status', Colors.CYAN)}            Show simulation status")
    print(c("│", Colors.BRIGHT_BLACK) + f"    {c('time', Colors.CYAN)}              Show current sim time")
    print(c("│", Colors.BRIGHT_BLACK) + f"    {c('personas', Colors.CYAN)}          List all personas")
    print(c("│", Colors.BRIGHT_BLACK))
    print(c("│", Colors.BRIGHT_BLACK) + c("  Persona Info:", Colors.BRIGHT_WHITE, Colors.BOLD))
    print(c("│", Colors.BRIGHT_BLACK) + f"    {c('schedule <name>', Colors.MAGENTA)}    Show persona's schedule")
    print(c("│", Colors.BRIGHT_BLACK) + f"    {c('location <name>', Colors.MAGENTA)}    Show persona's location")
    print(c("│", Colors.BRIGHT_BLACK) + f"    {c('memory <name>', Colors.MAGENTA)}      Show persona's memories")
    print(c("│", Colors.BRIGHT_BLACK) + f"    {c('chat <name>', Colors.MAGENTA)}        Start chat with persona")
    print(c("│", Colors.BRIGHT_BLACK))
    print(c("│", Colors.BRIGHT_BLACK) + f"    {c('help', Colors.BRIGHT_WHITE)}              Show this help")
    print(c("│", Colors.BRIGHT_BLACK))
    print(c("└────────────────────────────────────────────────────────────────┘", Colors.BRIGHT_BLACK))
    print()
...
def print_step_start(step, curr_time):
    ...
    print(c(f"▶ Step {step}", Colors.GREEN, Colors.BOLD) + c(f" @ {time_str}", Colors.DIM))
...
def print_persona_action(persona_name, action, emoji=""):
    ...
    print(f"  {c('●', color)} {c(persona_name, color, Colors.BOLD)}: {action}{emoji_str}")
...
def print_schedule(persona_name, schedule_items):
    ...
    print(c(f"┌─ {persona_name}'s Schedule ─────────────────────────────────────┐", Colors.MAGENTA))
...
def print_memory_summary(persona_name, events_count, thoughts_count, chats_count):
    ...
    print(c(f"┌─ {persona_name}'s Memory ───────────────────────────────────────┐", Colors.BLUE))
...
def get_prompt():
    ...
    try:
        return input(c("claudeville", Colors.CYAN, Colors.BOLD) + c(" ▸ ", Colors.BRIGHT_BLACK)).strip()
    except EOFError:
        return "exit"
...
def print_startup_menu(default_fork, last_sim):
    ...
    if last_sim:
        print(f"    {c('[c]', Colors.YELLOW, Colors.BOLD)}      Continue last simulation")
...
def print_simulation_started(sim_name):
    ...
    print(c("═" * 66, Colors.GREEN))
    print()
    print(c("  Frontend: ", Colors.DIM) + c("http://localhost:8000/simulator_home", Colors.BRIGHT_CYAN, Colors.BOLD))
    print()
    print(c("  Type 'help' for available commands, or 'run <N>' to simulate steps.", Colors.DIM))
    print()
...
def print_run_progress(current, total, persona_statuses):
    ...
    sys.stdout.write(f"\r  {c('[' + bar + ']', Colors.GREEN)} {c(f'{pct}%', Colors.BRIGHT_WHITE)} ({current}/{total})")
    sys.stdout.flush()
...
def print_run_complete(steps, total_time_sec):
    ...
    print()
    print(c(f"  ✓ Completed {steps} steps in {total_time_sec:.1f}s", Colors.BRIGHT_GREEN, Colors.BOLD))
    print()
