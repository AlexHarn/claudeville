...
def print_run_prompts(
    prompt_template=None, persona=None, prompt_input=None, prompt=None, output=None
):
    """Print prompt debug info in a clean, compact format."""

    # Extract just the function name from the template path
    func_name = (
        prompt_template.split("/")[-1].replace(".txt", "")
        if prompt_template
        else "unknown"
    )
    ...
    if HAS_CLI:
        ...
        print(
            cli.c(f"  ⚙ {func_name}", cli.Colors.DIM)
            + cli.c(" → ", cli.Colors.BRIGHT_BLACK)
