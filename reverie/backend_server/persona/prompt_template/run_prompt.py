...
File: run_prompt.py
Description: Defines all run prompt functions. These functions directly
...
def run_gpt_prompt_wake_up_hour(persona, test_input=None, verbose=False):
    ...
    output = safe_generate_response(
        prompt, 5, fail_safe, __func_validate, __func_clean_up
    )
...
def run_gpt_prompt_daily_plan(persona, wake_up_hour, test_input=None, verbose=False):
    ...
    # Note: The prompt already includes "wake up..." as item 1, so we don't prepend it again
    # Only add it if the model didn't include it (check if first item mentions "wake up")
    if not output or "wake up" not in output[0].lower():
        output = [
...
def run_gpt_prompt_task_decomp(persona, task, duration, test_input=None, verbose=False):
    ...
    output = safe_generate_response(
        prompt, 5, get_fail_safe(), __func_validate, __func_clean_up
    )
